...
public class MaximumSumOfThreeNonOverlappingSubarrays {
    ...
    class SolutionRevisedOnDayThird {
        ...
        private void findMaxSumPath(int[] nums, int k, int position, int count, int pathIdx, int[] path,
            ...
            int dontStart = findMaxSum(nums, k, position + 1, count + 1, prefixSum, mem);
            int start = findMaxSum(nums, k, position + 1, count + 1, prefixSum, mem) +
                    prefixSum[position + k] - prefixSum[position];
            ...
            if (start >= dontStart) {
                ...
                findMaxSumPath(nums, k, position + k, count + 1, pathIdx, path, prefixSum, mem);
    ...
    class SolutionRevisedOnDaySeventh {
        ...
        public int[] maxSumOfThreeSubarrays(int[] nums, int k) {
            ...
            findMaxSumPath(nums, k, 0, 0, 0, prefixSum, mem, path);
        ...
        private void findMaxSumPath(int[] nums, int k, int position, int count, int idx, int[] prefixSum, int[][] mem,
                int[] path) {
            if (count == 3) {
                return;
            }
            ...
            if (start >= dontStart) {
                path[idx] = position;
                findMaxSumPath(nums, k, position + k, count + 1, idx + 1, prefixSum, mem, path);
            } else {
                findMaxSumPath(nums, k, position + k, count, idx, prefixSum, mem, path);
            }
        }
    }
    ...
    class SolutionRevisedOnDayFourteen {
        ...
        private void findMaxSumPath(int[] nums, int k, int position, int count, int pathIdx, int[] path,
            ...
            int dontStart = findMaxSum(nums, k, position, count, prefixSum, mem);
            int start = findMaxSum(nums, k, position + k, count + 1, prefixSum, mem) +
                    prefixSum[position + k] - prefixSum[position];
